...
import { getPersonalJobs, savePersonalJobs, getEmployeeSimple } from '@/api/employees'
...
export default {
  ...
  methods: {
    ...
    // 获取员工岗位信息
    async getPersonalJobs() {
      this.formData = await getPersonalJobs(this.userId)
    },
    // 保存员工岗位信息
    async saveJob() {
      await savePersonalJobs(this.formData)
