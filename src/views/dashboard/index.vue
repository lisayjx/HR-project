      ...
      <div>
        <div class="fl headL">
          ...
          <div class="headInfoTip">
            <p class="firstChild">{{ username }},祝你开心每一天！</p>
            <p class="lastChild">{{ username }} | {{ userInfo.departmentName }}</p>
          </div>
        </div>
        <div class="fr" />
      </div>
    ...
    <el-dialog :visible="showDialog" title="离职申请" @close="btnCancel">
      ...
      >
        ...
        <el-form-item label="离职时间" prop="exceptTime">
          <!--type="datetime"+ value-format="yyyy-MM-dd HH:mm:ss"返回的时间就是字符串年月日时分秒了 -->
          <el-date-picker
  ...
  methods: {
    ...
    btnOK() {
      this.$refs.ruleForm.validate(async validate => {
        if (validate) {
          const data = { ...this.ruleForm, userId: this.userInfo.userId, username: this.username }
